...
export function ProgressBar({ yesCount, noCount, totalAnswered }: ProgressBarProps) {
  ...
  return (
      <div className="space-y-4 p-4 sm:p-6 bg-card rounded-xl shadow-elegant border border-gold/30 backdrop-blur-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h3 
            className="font-bold text-foreground text-lg sm:text-xl bg-gradient-to-r from-primary to-gold bg-clip-text text-transparent" 
            style={{ color: localStorage.getItem("aprovaclt_custom_approval_chances") || "#1e40af" }}
          >
            Chances de Aprovação
          </h3>
          <div className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full shadow-lg ${bgColor} ${textColor} animate-slide-up self-end sm:self-auto`}>
            {icon}
        ...
        <div className="space-y-3">
          <div className="w-full h-3 sm:h-4 bg-muted rounded-full overflow-hidden shadow-inner">
            <div 
        ...
        <div className={`text-center p-3 sm:p-4 rounded-xl shadow-inner transition-all duration-300 ${
          ...
          <p className="font-bold text-sm sm:text-lg">{message}</p>
        </div>
      </div>
  );
}